    ...
    def pred_save(self):
        ...
        with torch.no_grad():
            for iter, (name, origin, image, bboxes) in enumerate(self.loader):
                ...
                H,W,C       = origin.shape
                pred, pred2 = self.model(image, label)
                pred = pred[0, label, :, :].cpu().numpy()
                pred, gaus = self.gaussian_aug(pred)
                ...
                ## save
                path = '/data/code/zhaofan/CPCL' + '/bird/'+name.split('/')[0]
                if not os.path.exists(path):
                    os.makedirs(path)
                cv2.imwrite('/data/code/zhaofan/CPCL' + '/bird/' + name.replace('.jpg', '.png'), np.uint8(fuse))
                if iter % 100 == 0:
                    print(iter)
